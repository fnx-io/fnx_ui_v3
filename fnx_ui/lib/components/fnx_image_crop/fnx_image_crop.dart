...
class FnxImageCrop implements OnInit, AfterChanges, OnDestroy {
  ...
  StreamController<Rectangle<double>> _crop = StreamController();
  @Output()
  ...
  void emitCropResult() {
    ...
    var left = (imgOffsetX / imgWidth).abs();
    var top = (imgOffsetY / imgHeight).abs();
    var width = maskWidth / imgWidth;
    var height = maskHeight / imgHeight;
    if (top + height > 1) height = 1 - top;
    _crop.add(Rectangle(left, top, width, height));
  }
  ...
  void moveImageTouch(TouchEvent event) {
    ...
    // move
    try {
      var dx = event.touches.first.client.x - _lastTouch.touches.first.client.x;
      var dy = event.touches.first.client.y - _lastTouch.touches.first.client.y;
  ...
  Point<double> getCenter() {
    var cx = (maskWidth / 2.0 - imgOffsetX) / imgWidth.toDouble();
